...
(function ($, window) {
    ...
    functionality.filter = function (parameter) {
        ...
        $.each(configure.newCityData, function (key, value) {
            //匹配城市数据
            switch (value.letter.substring(0,1)) {
                case ('A'):
    ...
    functionality.search = function (event) {
        ...
        if (keyCode !== 13 && keyCode !== 38 && keyCode !== 40) {
            ...
            $.each(self.newCityData, function(key, value) {
                //拼音或者名称搜索
                if(value.pinyin.toLocaleLowerCase().search(inputVal.toLocaleLowerCase()) > -1 || value.name.search(inputVal) > -1 || value.id.search(inputVal) > -1 || value.letter.toLocaleLowerCase().search(inputVal.toLocaleLowerCase()) > -1){
                    result.push(value);
                }
            });
    ...
    functionality.singleResize = function () {
        ...
        $selector.find('.city-input').width(_iw - _sw - _p - 20);
    }
